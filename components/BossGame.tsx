...
export default function BossGame() {
  ...
  useEffect(() => {
    ...
    const handleInput = (e: any) => {
      ...
      const now = Date.now();
      // SINGLETON INPUT: Only one jump per 150ms
    ...
    const update = () => {
      if (jumpRequested.current) {
        if (boss.grounded) {
          ...
          boss.grounded = false;
          if (jumpSound.current) { jumpSound.current.currentTime = 0; jumpSound.current.play().catch(() => {}); }
        }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black p-2 md:p-4 select-none touch-none overflow-hidden">
      ...
      {/* GAME WINDOW: RESPONSIVE HEIGHT/WIDTH */}
      <div className="relative w-full max-w-[95vw] md:max-w-4xl aspect-[8/3] bg-black rounded-xl md:rounded-3xl border border-[#14F195]/20 shadow-2xl overflow-hidden">
        ...
        {/* SCORE HUD */}
        <div className="absolute top-3 left-3 md:top-6 md:left-6 z-50 bg-black/80 px-4 py-1 rounded-xl border border-white/10 pointer-events-none font-mono text-xl md:text-3xl text-[#14F195]">
          {displayScore}M
        </div>

        {/* MUTE TOGGLE */}
        <button onClick={() => setIsMuted(!isMuted)} className="absolute top-3 right-3 z-[250] bg-black/60 p-2 rounded-lg border border-white/10 text-[10px] text-white">
          {isMuted ? "🔇" : "🔊"}
        </button>

        {/* MENU: SCALED FOR MOBILE */}
        {!gameStarted && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/95 z-[200] p-6">
            <h2 className="text-white text-2xl md:text-4xl font-black italic mb-6 uppercase tracking-tighter">SELECT MODE</h2>
            ...
            <button onClick={() => setGameStarted(true)} className="px-12 py-4 md:px-20 md:py-6 rounded-2xl bg-white text-black font-black text-xl md:text-3xl hover:bg-[#14F195] transition-all transform active:scale-95 shadow-xl">START MISSION</button>
          </div>
        )}
        ...
        {gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/95 z-[220] p-6">
            <h4 className="text-red-600 text-6xl md:text-8xl font-black italic mb-8 uppercase tracking-tighter shadow-red-500/50">REKT</h4>
      ...
      {/* STICKY SOCIALS */}
      <div className="fixed bottom-6 right-6 flex flex-col gap-4 z-[300]">
        <a 
          href="https://t.me/Boss_Solana_Bull_Official" 
          target="_blank" 
          rel="noopener noreferrer" 
          className="bg-[#229ED9] p-4 rounded-full text-white shadow-[0_0_20px_rgba(34,158,217,0.5)] border border-white/20 hover:scale-110 active:scale-90 transition-all"
        >
